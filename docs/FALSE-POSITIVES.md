...
| `MEMORY-0003` on a small C utility | `-fstack-protector-strong`, the distribution default, instruments only functions with local arrays or address-taken locals. A program with none is compiled correctly and still carries no `__stack_chk_fail`. On a stock Debian host `/usr/bin/newgrp` is exactly this. |
| `MEMORY-0004` on Alpine, or anywhere musl replaces glibc | musl does not implement `_FORTIFY_SOURCE`'s `_chk` entry points, so no binary linked against it can reference one. On Alpine the golden bundle records `/usr/bin/crontab` and `/usr/bin/passwd` failing, and both are the same `/bin/busybox` under two names. The check is reporting the symbol table correctly; glibc's mechanism is simply not the one in use. |
