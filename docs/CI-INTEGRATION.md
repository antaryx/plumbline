...
Run as root, or accept that a large part of the host will report `UNKNOWN`.
...
**Pin the version and verify it.** A CI step that pipes an unverified download
into a root shell is a supply-chain hole in the pipeline that is auditing your
hosts for supply-chain holes.
...
counted in the run's invocation rather than emitted as results, because
seventy-four passing checks bury the three that matter. Accepted risks arrive as
native SARIF suppressions carrying their justification, so a dismissal in GitHub
shows the reason a human wrote. Full mapping: [`adr/0018-sarif-mapping.md`](adr/0018-sarif-mapping.md).
...
genuinely quiet. `REGRESSED` catches an expired suppression — a finding that
was accepted, whose acceptance lapsed, and which nobody would otherwise revisit.
...
Set `expires_at` on every rule. It is measured against the scan's start time,
so an accepted risk resurfaces on schedule rather than silently outliving the
person who accepted it.

## What not to do

- **Do not parse the terminal report.** Its layout may change in a patch
  release. Parse `--json`; it is schema-validated in CI and it is the API.
- **Do not gate on posture alone.** It is a ratio over *applicable* checks, and
  a narrow profile makes it flattering. Pair it with `--min-coverage`.
