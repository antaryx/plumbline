...
- Checks are pure functions, testable from fixtures. 79 checks are covered by a
  fixture corpus and six recorded distributions rather than by manual runs.
...
its own description, and passing it is evidence of hardening, not of
compliance.

### Why does it not fix anything?

There is no `--fix` flag, and [ADR-0006](adr/) records the decision not to add
one. A tool that both judges and modifies a host produces judgements that cannot
be audited, and remediation on a production host is a decision for someone with
the surrounding context. Plumbline prints the exact commands and the cautions
that go with them; `plumbline explain CHECK-ID` gives the full procedure.

### Can I run it unprivileged?

Yes, and the report says what it cost. Checks that could not read what they
needed report `UNKNOWN` rather than passing, so coverage falls. Use
`--strict-privileges` to exit 10 instead of producing a report that covers half
the host.
...
- Genuine ambiguity: for example `/etc/nsswitch.conf` says accounts come from
  LDAP, so the local files are not the whole picture and an unresolvable uid
  cannot be called stray.
...
because it cost thirteen transitive modules for a box border. The SBOM published
with every release makes the count checkable.

### Can I write my own checks?

Not at v1.0.0. Declarative check packs and a subprocess extension protocol are
v2 and v3 work in [`ROADMAP.md`](ROADMAP.md), deferred until there is enough
field use to know what shape the extension point should be. Profiles select from
the catalog; they cannot add to it, because a finding that exists only under one
profile cannot be reproduced by anyone else.

### Does it support macOS or BSD?

No. Linux only, glibc and musl. macOS is v3 work and will be announced only once
...
schema-validated in CI. The terminal layout may change in a patch release, which
