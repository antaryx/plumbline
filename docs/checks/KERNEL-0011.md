...
The running value of `fs.protected_fifos` is `1` or greater.

## 2. Why it matters

A program that creates a file in `/tmp` with `O_CREAT` gets whatever is already at that path. If an attacker put a FIFO there first, the program does not create a file — it opens the attacker's pipe. Writing to it blocks until the attacker chooses to read, which hangs the program; reading from it returns whatever the attacker decided to supply, which the program then trusts as its own data.
...
| Commands | `sysctl -w fs.protected_fifos=1` |
...
| `kernel-partial` | value 2 | PASS, detail names group-writable coverage |
